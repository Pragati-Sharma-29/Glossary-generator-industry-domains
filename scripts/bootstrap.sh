...
CORPUS_DISPLAY_NAME="${VERTEX_RAG_CORPUS_DISPLAY_NAME:-industry-glossaries}"
BUCKET=""
PORT="${PORT:-8080}"
...
Usage: $0 [flags]
  ...
  --corpus-name NAME      RAG corpus display name (default: $CORPUS_DISPLAY_NAME)
...
# ── 4. Build RAG corpus if missing ──────────────────────────────────────────
if [[ $SKIP_CORPUS -eq 0 ]]; then
  step "Checking for RAG corpus '$CORPUS_DISPLAY_NAME'"
  set +e
  python3 - "$PROJECT_ID" "$LOCATION" "$CORPUS_DISPLAY_NAME" <<'PYEOF'
import sys
import vertexai
from vertexai.preview import rag

project, location, display_name = sys.argv[1:4]
...
PYEOF
  ...
  if [[ $CORPUS_RC -eq 0 ]]; then
    ok "RAG corpus already exists — skipping build"
  elif [[ $CORPUS_RC -eq 2 ]]; then
    warn "Corpus not found — building (~5–15 min)"
    ...
    die "RAG corpus check failed unexpectedly"
  fi
else
...
export VERTEX_RAG_CORPUS_DISPLAY_NAME="$CORPUS_DISPLAY_NAME"
